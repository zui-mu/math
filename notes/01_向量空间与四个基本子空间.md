...
**rank one** 的含义是“矩阵的秩等于 $`1`$”，不是“矩阵的第一列”。它表示矩阵的列空间只有一个独立方向：只需一个非零列向量，就能张成全部列。
...
```math
\mathbf{a}_2=4\mathbf{a}_1,
\qquad
\mathbf{a}_3=5\mathbf{a}_1.
```
...
```math
\dim\bigl(\mathrm{Col}(A)\bigr)=1,
\qquad
\mathrm{rank}(A)=1.
```

这再次体现了三者的区别：**基**是那个具体的向量集合，**维数**是基中向量的数量，**秩**是矩阵列空间的维数。

> **rank 是一个数字，basis 是一组向量。对于列空间，rank 等于任意一组 basis 所含向量的数量。**
...
\qquad
\mathbf{v}^T=\begin{bmatrix}1&4&5\end{bmatrix},
```
...
这个乘积称为**外积**（outer product）。它把一个 $`m\times1`$ 列向量与一个 $`1\times n`$ 行向量相乘，得到一个 $`m\times n`$ 矩阵：

```math
\mathbf{u}\mathbf{v}^T
=
\begin{bmatrix}
v_1\mathbf{u}&v_2\mathbf{u}&\cdots&v_n\mathbf{u}
\end{bmatrix}.
```

每一列都是同一个向量 $`\mathbf{u}`$ 的倍数，所以只要 $`\mathbf{u}`$ 与 $`\mathbf{v}`$ 都非零，这个矩阵的秩就是 $`1`$。反过来，任意非零的秩为 $`1`$ 的矩阵，其所有列都在同一个独立方向上，因此都能写成
...
这里的 $`\mathbf{u}`$ 不必是第一列。第一列可能恰好为零，真正提供独立方向的主元列也可能出现在后面。例如，
...
仍然是秩为 $`1`$ 的矩阵，它的第二列可以作为列空间的一组基。一般地，$`\mathrm{rank}(A)=r`$ 表示需要 $`r`$ 个独立列方向张成全部列，而不是指前 $`r`$ 列一定独立。

秩为 $`r`$ 的矩阵还可以看成 $`r`$ 个秩为 $`1`$ 的外积之和：

```math
A
=\mathbf{u}_1\mathbf{v}_1^T
+\cdots+
\mathbf{u}_r\mathbf{v}_r^T.
```

这描述的是矩阵的分解，并不表示任意两个矩阵相加时秩也会直接相加。

后文将沿着“主元列 $`\rightarrow`$ 列空间的基 $`\rightarrow`$ 维数 $`\rightarrow`$ 秩”这条关系，用高斯消元系统地识别主元结构。在此之前，先把同样的基与维数概念应用到“矩阵本身作为向量”的空间。
...
| 所有秩为 $`1`$ 的矩阵 | 否 | 不包含零矩阵，两个秩为 $`1`$ 的矩阵相加后秩也未必仍为 $`1`$ |
| 所有正定实对称矩阵 | 否 | 不包含零矩阵，负数倍也不再正定 |

奇异矩阵的一个 $`2\times2`$ 反例是：$`E_{11}`$ 与 $`E_{22}`$ 都奇异，但两者之和 $`I_2`$ 可逆。**包含零向量只是子空间的必要条件，不是充分条件。**

秩为 $`1`$ 的矩阵集合也不是子空间。首先，零矩阵的秩为 $`0`$，所以它不在这个集合中；其次，

```math
A=E_{11}=\begin{bmatrix}1&0\\0&0\end{bmatrix},
\qquad
B=E_{22}=\begin{bmatrix}0&0\\0&1\end{bmatrix}
```

都满足 $`\mathrm{rank}(A)=\mathrm{rank}(B)=1`$，但
...
所以这个集合既不包含零矩阵，也不对加法封闭。这里不能把秩当作普通数直接相加；一般只能保证
...
两个秩为 $`1`$ 的矩阵相加，结果的秩可能是 $`0`$、$`1`$ 或 $`2`$。这与子空间维数公式

```math
\dim(S+U)=\dim S+\dim U-\dim(S\cap U)
```

不是同一件事：前者研究两个**具体矩阵相加后的秩**，后者研究两个**子空间之和的维数**。
...
对单个矩阵而言，列空间的一组基给出具体的独立列方向，基中向量的数量就是列空间的维数，也就是矩阵的秩：
...
秩为 $`1`$ 时，全部列只有一个独立方向，因此矩阵可以写成一个列向量与一个行向量的外积：
...
一般的秩为 $`r`$ 的矩阵可以分解成 $`r`$ 个秩为 $`1`$ 的外积之和，但“所有秩为 $`1`$ 的矩阵”并不构成子空间，因为它不包含零矩阵，也不对加法封闭。
...
> **存在性：** $`r=m`$ 表示每一行都有主元，因此每个右端向量都有解。
>
> **唯一性：** $`r=n`$ 表示每一列都有主元，因此方程有解时解唯一。
...
至此，第一部分形成了一条完整主线：从线性组合、张成、线性无关、基和维数出发，经由矩阵方程与高斯消元得到主元、自由变量和秩，再由转置补齐行空间与左零空间，最终用四个基本子空间统一描述矩阵的输入、输出、可达方向与丢失方向。后续课程内容将在同一仓库的 `notes` 目录中使用新的编号文件继续整理。
...
| rank-one matrix | 秩为 $`1`$ 的矩阵 | 列空间只有一个独立方向的非零矩阵 |
| outer product | 外积 | 列向量与行向量的乘积 $`\mathbf{u}\mathbf{v}^T`$ |
